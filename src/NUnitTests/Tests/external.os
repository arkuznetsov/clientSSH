...
Процедура ПередЗапускомТестов()
	...
	ТекущийКаталог = ПолучитьПеременнуюСреды("SSH_TEST_CWD");
	...
	ПутьККомпоненте = ПолучитьПеременнуюСреды("SSH_TEST_LIB_PATH");
	Если НЕ ЗначениеЗаполнено(ПутьККомпоненте) Тогда
		ПутьККомпоненте = ОбъединитьПути(ТекущийКаталог, "src", "clientSSH", "bin");
		ПутьККомпоненте = ОбъединитьПути(ПутьККомпоненте, "Debug", "net452", "ClientSSH.dll");
	КонецЕсли;
...
Процедура ТестДолжен_Упасть() Экспорт
	...
	Сообщить("Этот тест должен упасть!");
